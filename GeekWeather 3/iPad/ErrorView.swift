    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                Spacer()
                HStack {
                    Image(systemName: "xmark.octagon.fill").resizable().frame(width: 75, height: 75, alignment: .center).padding().foregroundColor(.white)
                    Text(error.title)
                        .font(Font.custom("AvenirNext-Medium", size: 50))
...
struct ErrorView_Previews: PreviewProvider {
    static var previews: some View {
        ErrorView(error: WeatherFetcherError(title: "Error", description: "Something went wrong")).previewDevice(PreviewDevice(rawValue: "iPad Pro (12.9-inch)"))
    }
}
